//this file is no longer necessary, but it could potentially be useful for people using it for other curves
...
forward find_and_test_model_2;

function find_and_test_model_new(x_num, x_denom, y_num, y_denom, domain_curve, x, y, known_rat_pts : primes_to_test := [], max_prime := 50, printlevel := 1)
  ...
  if #image_bad_Q_pts eq 0 then
      ...
      for p in primes_to_test do
        ...
        for PQred in PQreds do
          ...
          return find_and_test_model_2(new_model_map, known_rat_pts : primes_to_test := primes_to_test, max_prime := max_prime, printlevel := printlevel);
        end for;
      end for;
      print "\n\nDone.";
      return [];
...
function find_and_test_model_2(model_map, known_rat_pts : primes_to_test := [], max_prime := 50, printlevel := 1)
